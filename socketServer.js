...
const SocketServer = (socket) => {
  ...
  socket.on('joinUser', (user) => {
    user.push({ id: user._id, socketId: socket.id, followers: user.followers });
  });

  socket.on('disconnect', () => {
    const data = users.find((user) => user.socketId === socket.id);
    if (data) {
      const clients = user.filter((user) =>
  ...
  socket.on('createComment', (newPost) => {
    ...
    if (clients.length > 0) {
      clients.forEach((client) => {
        socket.to(`${client.socketId}`).emit('createCommitToClient', newPost);
      });
    }
  });
  ...
  socket.on('checkUserOnline', (data) => {
    const following = users.find((user) =>
      data.following.find((item) => item.id === user.id)
    );
